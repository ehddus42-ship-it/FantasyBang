...
- 완전 체험: 사람 1명+AI 2~5명의 재시작 가능한 3~6인 완결 단판, 목표 10~20분.
...
소셜 표현은 사람 1명+AI 2~5명의 순수 싱글이다. AI를 온라인 사람으로 표시하지 않는다. 모든 좌석은 사람/AI로 교체할 수 있다.
...
포함: 3~6인 완결 단판, 10영웅, 정확히 60장의 9카드군, 역할 추리, 3인 공개 역할 특수전, 부관 규칙, 원형 거리, 즉시 반응, AI, 같은/새 시드 재시작, 공개/좌석별 개인 뷰, 사람/AI 좌석 교체, 결정적 이벤트, 로컬 전송과 Firebase 전달 계약, 반응형 UI, WebAudio 피드백, 접근성/저모션, Node 테스트와 오토플레이.

제외: 실제 Firebase, 계정, 온라인 방, 장기 성장, 결제, 장기 밸런스 확정.
...
- 성능·용량: 로컬 브라우저 내비게이션 38–50ms, 사용자 입력→두 번째 페인트 p50 27.9ms / p95 30.6ms / 최악 30.9ms, long task 0건이다. 폴리싱 증거를 포함한 전체 파일은 3,778,800 bytes로 20MB 상한 이내다.
...
- 증가: 없음.
- 감소: 실제 Firebase 네트워크는 계획대로 제외했다. 3~6인 로컬 AI전과 부관 실전 배치는 포함했다. 발행 이미지 11개는 Meshy AI가 생성했으며 코드는 결손 폴백도 유지한다.
- 미완 검증: Safari 고유 런타임과 실제 Firebase 네트워크. Chromium 시각/입력/반응형, 헤드리스 규칙·멀티플레이 계약·200시드 권위 시뮬레이션은 통과했다.

## 폴리싱 반영 — 2026-08-11

- 표시 이름: `반역`
- 주 플레이 기준: 1440×900 PC 데스크톱. 전장·역할 정보·손패의 동시 판독을 우선하고 모바일은 보조 호환으로 유지한다.
- 쉬운 역할명: 왕, 부관, 반역자, 야심가
- 쉬운 카드명: 공격, 방어, 회복, 사거리 강화, 카드 파괴, 카드 훔치기, 결투, 전체 공격, 추가 뽑기
- 영웅 초상: Meshy AI `nano-banana-2`로 영웅 10종을 일본 라이트노벨·모바일 RPG풍 1024×1024 이미지로 재생성했다.
- 재검증: 단위·규칙·멀티플레이 계약 테스트 8/8 PASS, 데스크톱·모바일 Chromium 시각 점검 PASS.
- 목표 지정 UI: 대상 카드 선택 뒤 전장에서 강조된 캐릭터 좌석을 직접 클릭하거나 Enter·Space로 선택한다. 대상별 하단 버튼은 표시하지 않는다.
- 처치 보상: 마지막 피해로 캐릭터를 탈락시킨 플레이어 좌석 하나만 즉시 카드 3장을 뽑는다. 전용 테스트를 포함한 전체 규칙 테스트 9/9와 200시드 자동 플레이를 통과했다.
- 인원·부관: 3~6인 선택을 지원한다. 3인은 공개 역할 순환 목표 특수전, 4인은 부관 없음, 5~6인은 왕 팀 부관 1명을 사용한다. 전체 규칙 테스트 13/13, 인원별 16시드 완주, 4인 200시드 밸런스 게이트를 통과했다.
