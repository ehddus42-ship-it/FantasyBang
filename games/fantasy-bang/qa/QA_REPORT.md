...
## 17. 목표 캐릭터 직접 선택 — 2026-08-11

- 공격·회복·교란처럼 목표가 필요한 카드를 선택하면 합법적인 목표 캐릭터 좌석만 파란빛으로 강조한다.
- 턴 종료 버튼 옆에 나열하던 대상별 행동 버튼을 제거했다. 강조된 캐릭터를 클릭하거나 키보드 Enter·Space로 선택하면 해당 카드 행동을 즉시 적용한다.
- 대상이 없는 카드, 방어 반응, 손패 정리, 영웅 능력, 턴 종료는 기존 행동 버튼을 유지한다.
- Chromium 1440×900에서 공격 카드 선택 → 목표 좌석 2개 표시 → 마우스 클릭 및 Enter 적용 → 행동 기록 갱신을 확인했다. 가로·세로 넘침은 0이다.
- 화면 증거: `24-target-character-selection-desktop.jpg`.

## 18. 처치자 3장 보상 — 2026-08-11

- 생명력을 0으로 만든 마지막 피해의 출처 좌석 하나만 즉시 카드 3장을 뽑는다.
- 다른 생존자에게는 카드를 지급하지 않으며, 출처 없는 탈락과 자기 자신을 처치한 경우에는 보상이 없다.
- 처치 보상은 `killReward` 행동 기록으로 남아 `처치 보상 카드 +3`으로 표시한다.
- 전용 규칙 테스트를 추가해 처치자의 사용 카드 1장 소모 뒤 정확히 3장 증가, 다른 두 생존자의 손패 불변, 탈락자의 손패 폐기를 확인했다.
- 전체 테스트 9/9 PASS, 128시드 완주 PASS, 200시드 자동 플레이 밸런스 게이트 PASS.

## 19. 3~6인 역할 구성·부관 — 2026-08-11

- 3인: 부관 1·반역자 1·야심가 1, 전 역할 공개. 부관→야심가→반역자→부관 순서로 자기 목표를 직접 처치하면 즉시 승리한다. 다른 역할이 대신 처치하면 남은 둘이 최후 생존전을 벌인다.
- 4인: 왕 1·반역자 2·야심가 1. 부관 없음.
- 5인: 왕 1·부관 1·반역자 2·야심가 1.
- 6인: 왕 1·부관 1·반역자 3·야심가 1.
- 부관은 왕 승리 시 생존 여부와 무관하게 함께 승리한다. 왕이 부관을 처치하면 3장 처치 보상을 받지 않고 손패와 장비를 모두 버린다.
- 전체 규칙 테스트 13/13 PASS. 기본 4인 128시드, 3~6인 각 16시드 총 64판 완주, 4인 권위 시뮬레이션 200시드 PASS.
- Chromium 1440×900에서 3·4·5·6인 좌석 수, 공개 역할, 가로·세로 넘침 0, 좌석 겹침 0을 확인했다.
- 화면 증거: `25-three-player-desktop.jpg`, `26-six-player-desktop.jpg`.
